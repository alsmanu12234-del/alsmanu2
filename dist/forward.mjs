...
export async function handleForwardCallback(query) {
  ...
  if (data === "fw_chpic") {
    ...
    try {
      ...
      // [FIX-CHPIC] تنزيل الصورة كـ buffer لتجنب مشكلة auth headers مع تيليجرام
      const _picRes = await fetch(url, {
        headers: { "User-Agent": "WhatsApp/2.0" },
        signal: AbortSignal.timeout(12000),
      });
      if (!_picRes.ok) throw new Error("fetch " + _picRes.status);
      const _picBuf = Buffer.from(await _picRes.arrayBuffer());
      const { Readable: _PicReadable } = await import("stream");
      const _picStream = _PicReadable.from(_picBuf);
      await _bot.sendPhoto(chatId, _picStream, { caption: `🖼️ بروفايل: ${result.name}` }, { filename: "profile.jpg", contentType: "image/jpeg" });
    } catch {
